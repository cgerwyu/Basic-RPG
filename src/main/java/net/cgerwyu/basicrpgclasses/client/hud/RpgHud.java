...
public final class RpgHud {
    ...
    private static final Identifier GOTHIC_COMBAT_HUD_TEXTURE = Identifier.fromNamespaceAndPath(
            "basicrpgclasses", "textures/gui/gothic_combat_hud.png"
    );
    ...
    private static final int BOSS_BAR_WIDTH = 142;
    private static final int BOSS_BAR_HEIGHT = 52;
    ...
    public static void renderBossBar(
        ...
        int barX = x + Math.round(width * 18.0F / BOSS_BAR_WIDTH);
        int barY = y + 19;
        int barWidth = Math.max(8, Math.round(width * 105.0F / BOSS_BAR_WIDTH));
        drawInsetBar(graphics, barX, barY, barWidth, 14,
                Math.clamp(bossEvent.getProgress(), 0.0F, 1.0F),
                bossColor, darken(bossColor, 0.22F), brighten(bossColor, 0.60F));
        drawCombatAtlasRegion(graphics, x, y, width, height, 109, 156, 142, 52);
        String percent = Math.round(bossEvent.getProgress() * 100.0F) + "%";
        graphics.centeredText(minecraft.font, percent, barX + barWidth / 2, barY + 2, TEXT_PRIMARY);
        String bossName = fitText(minecraft, bossEvent.getName().getString(), width - 24);
        graphics.centeredText(minecraft.font, bossName, x + width / 2, Math.max(1, y - 9), TEXT_PRIMARY);
    ...
    private static void renderSkillBar(GuiGraphicsExtractor graphics, Minecraft minecraft) {
        ...
        int frameGap = 4;
        int slotStep = slot + frameThickness * 2 + frameGap;
        int panelWidth = 230;
        int panelHeight = 46;
        ...
        drawCombatAtlasRegion(graphics, panelX, panelY, panelWidth, panelHeight, 6, 208, 230, 46);

        for (int index = 0; index < CombatModeController.SKILL_SLOT_COUNT; index++) {
            int x = startX + index * slotStep;
            ...
            graphics.fill(x, y, x + slot, y + slot, selected ? 0xF13A2024 : 0xF0101015);
            if (selected) {
                graphics.outline(x, y, slot, slot, slotTrim);
            }
            drawHalfScaleText(graphics, minecraft, Integer.toString(index + 1), x + 1, y, slotTrim);

            if (definition == null) {
                drawCenteredHalfScaleText(graphics, minecraft, "—", x + slot / 2, y + 5, 0xFF827D81);
                continue;
            }

            int rank = data.skillRank(skillId);
            graphics.fill(x + 1, y + 4, x + slot - 1, y + 5, definition.color());
            if (ClientToggleSkillStates.active(skillId)) {
                drawActiveToggle(graphics, x, y, slot, gameTime);
            }
            drawCenteredHalfScaleText(graphics, minecraft,
                    Component.translatable(skillId.shortTranslationKey()).getString(),
                    x + slot / 2, y + 5, TEXT_PRIMARY);
            ...
            if (maxCharges > 1) {
                drawHalfScaleText(graphics, minecraft, availableCharges + "/" + maxCharges,
                        x + 1, y + 9, availableCharges > 0 ? TEXT_PRIMARY : 0xFF9D979A);
            }
            ...
            int resourceCostColor = combat.canAfford(displayedCost)
                    ...
                    : 0xFFFF6874;
            int costWidth = halfScaleTextWidth(minecraft, resourceCost);
            drawHalfScaleText(graphics, minecraft, resourceCost,
                    x + slot - costWidth - 1, y + 9, resourceCostColor);
    ...
    private static int halfScaleTextWidth(Minecraft minecraft, String text) {
        return (minecraft.font.width(text) + 1) / 2;
    }

    private static void drawCenteredHalfScaleText(
            GuiGraphicsExtractor graphics,
            Minecraft minecraft,
            String text,
            int centerX,
        ...
        drawHalfScaleText(graphics, minecraft, text,
                centerX - halfScaleTextWidth(minecraft, text) / 2, y, color);
    }

    /** Keeps key, abbreviation and cost legible without escaping a 14x14 opening. */
    private static void drawHalfScaleText(
            GuiGraphicsExtractor graphics,
            Minecraft minecraft,
            String text,
            int x,
    ...
    private static void drawCooldownRadar(
        ...
        if (size <= 14) {
            graphics.fill(x, y, x + size, y + size, locked ? 0xBA08070A : 0x7408070A);
            int curtainHeight = Math.clamp((int) Math.ceil(size * ratio), 0, size);
            graphics.fill(x, y, x + size, y + curtainHeight, 0xA808070A);
            String seconds = String.format(Locale.ROOT, "%.1f", Math.ceil(remainingTicks / 2.0) / 10.0);
            drawCenteredHalfScaleText(graphics, minecraft, seconds, x + size / 2, y + 5, TEXT_PRIMARY);
            return;
        }
    ...
    private static void drawReadyFlash(GuiGraphicsExtractor graphics, int x, int y, int size, float flash) {
        ...
        if (size <= 14) {
            graphics.outline(x, y, size, size, color);
            return;
        }
    ...
    private static void drawActiveToggle(GuiGraphicsExtractor graphics, int x, int y, int size, long gameTime) {
        ...
        if (size <= 14) {
            graphics.outline(x, y, size, size, glow);
            graphics.fill(x + 1, y + 1, x + 2, y + 2, 0xFFE6FFFF);
            graphics.fill(x + size - 2, y + size - 2, x + size - 1, y + size - 1, 0xFFE6FFFF);
            return;
        }
    ...
    private static void drawAtlasRegion(
        ...
        drawAtlasRegion(graphics, GOTHIC_HUD_TEXTURE, x, y, width, height,
                sourceX, sourceY, sourceWidth, sourceHeight);
    }

    private static void drawCombatAtlasRegion(
            GuiGraphicsExtractor graphics,
            int x,
            int y,
            int width,
        ...
        drawAtlasRegion(graphics, GOTHIC_COMBAT_HUD_TEXTURE, x, y, width, height,
                sourceX, sourceY, sourceWidth, sourceHeight);
    }
