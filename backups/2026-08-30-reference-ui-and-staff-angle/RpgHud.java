...
/**
 * A lightweight, resolution-independent HUD skin. It deliberately uses GUI primitives instead of a
 * texture atlas so the RPG HUD remains crisp with every GUI scale and resource pack.
 */
public final class RpgHud {
    ...
    private static final int BOSS_BAR_HEIGHT = 34;
    ...
    public static void renderBossBar(
        ...
        int width = Math.min(292, graphics.guiWidth() - 20);
        ...
        drawGothicPanel(graphics, x, y, width, height, bossColor);
        drawCrest(graphics, x + width / 2, y + 1, bossColor);

        String bossName = fitText(minecraft, bossEvent.getName().getString(), width - 38);
        graphics.centeredText(minecraft.font, bossName, x + width / 2, y + 7, TEXT_PRIMARY);
        ...
        drawGothicBar(
                ...
                bossColor,
                darken(bossColor, 0.22F),
                brighten(bossColor, 0.60F),
                gameTime,
                2
        );
        String percent = Math.round(bossEvent.getProgress() * 100.0F) + "%";
        graphics.text(minecraft.font, percent, x + width - 15 - minecraft.font.width(percent), y + 7, TEXT_MUTED, false);
    ...
    private static void renderPlayerFrame(GuiGraphicsExtractor graphics, Minecraft minecraft, LocalPlayer player) {
        ...
        drawGothicPanel(graphics, x, y, width, height, resourceColor);
        drawPortraitFrame(graphics, player, x + 7, y + 14, 40, resourceColor);
        drawCrest(graphics, x + width / 2, y + 1, resourceColor);

        graphics.text(minecraft.font, fitText(minecraft, player.getName().getString(), 111), x + 54, y + 12, TEXT_PRIMARY, false);
        graphics.text(
                minecraft.font,
        ...
        drawGothicBar(graphics, x + 54, y + 35, 111, 9, healthRatio,
                HEALTH_FILL, HEALTH_EMPTY, HEALTH_SHINE, gameTime, 0);
        ...
        drawGothicBar(graphics, x + 54, y + 48, 111, 8, resourceRatio,
                resourceColor, ClassResourceRules.barBackground(data.rpgClass()), resourceHighlight, gameTime, 7);
        graphics.centeredText(
                minecraft.font,
    ...
    private static void renderPartyFrames(GuiGraphicsExtractor graphics, Minecraft minecraft, LocalPlayer player) {
        ...
        for (int index = 0; index < members.size(); index++) {
            renderPartyMember(graphics, minecraft, members.get(index), x, y + 10 + index * 39);
        }
    }
    ...
    private static void renderPartyMember(
        ...
        int classColor = ClassResourceRules.barColor(data.rpgClass());
        long gameTime = member.level().getGameTime();
        int width = 150;
        int height = 35;
        drawGothicPanel(graphics, x, y, width, height, classColor);
        drawPortraitFrame(graphics, member, x + 5, y + 7, 22, classColor);

        graphics.text(minecraft.font, fitText(minecraft, member.getName().getString(), 108), x + 33, y + 7, TEXT_PRIMARY, false);
        ...
        graphics.text(minecraft.font, classLine, x + 33, y + 16, TEXT_MUTED, false);

        float healthRatio = Math.clamp(member.getHealth() / Math.max(1.0F, member.getMaxHealth()), 0.0F, 1.0F);
        drawGothicBar(graphics, x + 33, y + 26, 108, 6, healthRatio,
                HEALTH_FILL, HEALTH_EMPTY, HEALTH_SHINE, gameTime, 4);
    }
    ...
    private static void renderSkillBar(GuiGraphicsExtractor graphics, Minecraft minecraft) {
        ...
        int panelWidth = totalWidth + 22;
        int panelHeight = 46;
        int panelX = minecraft.getWindow().getGuiScaledWidth() / 2 - panelWidth / 2;
        int panelY = minecraft.getWindow().getGuiScaledHeight() - 72;
        int startX = panelX + 11;
        int y = panelY + 8;

        drawGothicPanel(graphics, panelX, panelY, panelWidth, panelHeight, 0xFFC2B9AC);
        drawCrest(graphics, panelX + panelWidth / 2, panelY + 1, 0xFFC2B9AC);
        ...
        for (int index = 0; index < CombatModeController.SKILL_SLOT_COUNT; index++) {
            ...
            graphics.fill(x, y, x + slot, y + slot, selected ? 0xE13A2024 : 0xDE101015);
            graphics.outline(x, y, slot, slot, slotTrim);
            graphics.outline(x + 2, y + 2, slot - 4, slot - 4, 0xAA2B292E);
            graphics.text(minecraft.font, Integer.toString(index + 1), x + 3, y + 3, slotTrim, false);

            if (definition == null) {
                graphics.centeredText(minecraft.font, "—", x + slot / 2, y + 11, 0xFF827D81);
                continue;
            }

            int rank = data.skillRank(skillId);
            graphics.fill(x + 3, y + 3, x + slot - 3, y + 5, definition.color());
            ...
            if (maxCharges > 1) {
                graphics.text(minecraft.font, availableCharges + "/" + maxCharges, x + 3, y + slot - 10,
            ...
            int resourceCostColor = combat.canAfford(displayedCost)
                    ...
                    : 0xFFFF6874;
            graphics.text(minecraft.font, resourceCost, x + slot - minecraft.font.width(resourceCost) - 3,
                    y + slot - 10, resourceCostColor, false);
